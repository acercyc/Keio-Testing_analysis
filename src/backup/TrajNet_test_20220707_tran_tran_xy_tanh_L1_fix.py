...
wandb_logger = WandbLogger(project="test_20220707", id='tran_tran_xy_tanh_L1_2')
...
class TrajNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.hidden(x)
        x = torch.tanh(x)
        self.L1_loss = torch.norm(x, 1) / x.numel()
...
class PL_model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('L1_loss', self.model.L1_loss) 
        self.log('train_loss', train_loss)
        self.log('alpha', self.model.alpha)
        alpha_ = torch.sigmoid(self.model.alpha)
        loss = train_loss * alpha_ + self.model.L1_loss * (1-alpha_)
        return loss
