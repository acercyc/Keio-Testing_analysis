...
wandb_logger = WandbLogger(project="test_20220707", id='lstm_big_noposition')
...
class Transformer(nn.Module):
    def __init__(self, nFeature=16, nhead=16, dim_feedforward=64, num_layers=8, dropout=0.1):
...
class TrajNet(nn.Module):
    def __init__(self, nHidden=8, nFeature=32, dropout=0.1):
        ...
        # encoding
        self.enc_conv = nn.Conv1d(2, nFeature, 1)
        self.encoder = Transformer(nFeature=nFeature, dropout=dropout)
        ...
        # decode
        self.dec_conv1 = nn.Conv1d(nHidden, nFeature, 1)
        self.decoder = Lstm(nFeature=nFeature, dropout=dropout)
        self.dec_conv2 = nn.Conv1d(nFeature, 2, 1)
