...
wandb_logger = WandbLogger(project="test_20220707", id='tran_tran_xy_tanh_small2')
...
class Transformer(nn.Module):
    def __init__(self, nFeature=64, nhead=16, dim_feedforward=128, num_layers=4, dropout=0.1):
...
class TrajNet(nn.Module):
    def __init__(self, nHidden=8, nFeature=32, dropout=0.1):
        super(TrajNet, self).__init__()
        self.nhidden = nHidden
        self.nFeature = nFeature

        # encoding
        self.enc_conv = nn.Conv1d(2+8, 32, 1)
        self.encoder = Transformer(nFeature=32, dropout=dropout)
        
        # hidden
        self.hidden = nn.Linear(32, nHidden)
        # self.silu = nn.SiLU()
        ...
        # decode
        self.dec_conv1 = nn.Conv1d(nHidden, 32-8-2, 1)
        self.decoder = Transformer(nFeature=32, dropout=dropout)
