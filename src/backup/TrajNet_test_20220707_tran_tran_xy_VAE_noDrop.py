...
wandb_logger = WandbLogger(project="test_20220707", id='tran_tran_xy_VAE_noDrop2')
...
class TrajNet(nn.Module):
    def __init__(self, nHidden=8, nFeature=64, dropout=0.01):
        super(TrajNet, self).__init__()
        self.nhidden = nHidden
        self.nFeature = nFeature

        # encoding
        self.enc_conv = nn.Conv1d(2+8, 64, 1)
        self.encoder = Transformer(nFeature=64, dropout=dropout)
        
        # hidden
        # self.hidden = nn.Linear(64, nHidden)
        self.mu = nn.Linear(64, nHidden)
        self.log_var = nn.Linear(64, nHidden)
        self.alpha = nn.Parameter(torch.tensor(0.0, requires_grad=True))    

        # decode
        self.dec_conv1 = nn.Conv1d(nHidden, 64-8-2, 1)
        self.decoder = Transformer(nFeature=64, dropout=dropout)
        self.dec_conv2 = nn.Conv1d(64, 2, 1)
    ...
    @staticmethod
    def positionEncoding_sincos(x, nDim=4):
        nBatch = x.shape[0]
        nTime = x.shape[1]
        mat = utils.DataProcessing.positionEncoding_sincos_mat(nTime, dim=nDim)
    ...
    def forward(self, x):
        ...
        # --------------------------------- encoding --------------------------------- #
        x = self.positionEncoding_sincos(x)
...
class PL_model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_loss = torch.nn.functional.mse_loss(y, batch)
        alpha_ = torch.sigmoid(self.model.alpha)
        loss = train_loss * alpha_ + self.model.kl_loss * (1-alpha_)
        self.log('train_loss', train_loss)
        self.log('kl_loss', self.model.kl_loss)
        self.log('alpha', self.model.alpha)   
        return loss
