...
if __name__ == '__main__':
    ...
    # path
    savepath = utils.path_data / 'TrajNet_train_60s_retrain'
    ...
    # dataloader 
    dataset_train, dataset_val = utils.LoadData.mouseMovementRollingData(subjID=subj, task=task, wSize=config['wSize'], pos=config['pos'], seed=1)
    ...
    wandb_logger = WandbLogger(project=project, 
                               name=f'{subj}_{task}_60s_retrain', 
                               save_dir=str(savepath_wb),
                               config=config, tags=tags, group='tran2lstm_60s')
